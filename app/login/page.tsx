...
export default function Login(){
  ...
  return <main className="app"><div className="auth-layout"><section className="auth-story"><BrandLogo/><p className="eyebrow section-gap">PIB Penilaian</p><h2>Lebih mudah menilai.<br/>Lebih jelas memantau.</h2><p>Ruang kerja penilaian Praktik Ibadah. Kelola siswa, catat hasil, dan siapkan laporan dalam satu tempat.</p></section><section className="auth-form"><p className="eyebrow">Selamat datang kembali</p><h1>Masuk ke ruang kerja</h1><p className="subtitle">Gunakan akun guru untuk melanjutkan penilaian.</p><form className="section-stack section-gap" aria-busy={busy} onSubmit={submit}><div className="field"><label htmlFor="email">Email</label><input id="email" type="email" autoComplete="username" placeholder="nama@sekolah.sch.id" value={email} onChange={e=>setEmail(e.target.value)} required/></div><div className="field"><label htmlFor="password">Password</label><div className="password-control"><input id="password" type={show?"text":"password"} autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} required/><button type="button" aria-label={show?"Sembunyikan password":"Tampilkan password"} aria-pressed={show} onClick={()=>setShow(v=>!v)}>{show?<EyeOff size={18}/>:<Eye size={18}/>}</button></div></div>{error&&<Alert type="error">{error}</Alert>}<button className="primary" disabled={busy}>{busy?"Memeriksa akun…":"Masuk"}<ArrowRight size={16}/></button><p className="hint">Akun dikelola oleh pengelola aplikasi di sekolah Anda.</p></form></section></div></main>;
}
