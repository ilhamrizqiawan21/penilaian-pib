...
export async function GET(){const user=await requireUser();if(isResponse(user))return user;const data=Object.fromEntries(tables.map((table)=>[table,db.prepare(`SELECT * FROM ${table}`).all()]));audit(user.id,"backup","local","DOWNLOAD","Backup data-only dibuat");return new NextResponse(JSON.stringify({version:2,schemaVersion:schemaVersion(),createdAt:new Date().toISOString(),app:"pib-penilaian",data}),{headers:{"Content-Type":"application/json","Content-Disposition":"attachment; filename=backup-pib.json","Cache-Control":"no-store"}})}
