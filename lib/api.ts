...
export function writeGuard(req:Request){const origin=req.headers.get("origin");return !origin||origin===new URL(req.url).origin?null:NextResponse.json({error:"Origin request tidak valid"},{status:403})}
